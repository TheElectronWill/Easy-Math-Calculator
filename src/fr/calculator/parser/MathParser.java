 ...
 * <h1>Représentation interne des données</h1> Une "expression mathématique" est définie comme une suite de termes qui s'additionnent pour
 * donner un résultat. Ces termes sont stockés sous la forme d'une liste, plus précisément {@code List<Term>}. Par exemple, le calcul
 * <code>"2 + 45 - 6"</code> donne la liste suivante: <code>[+2,+45,-6]</code>
 *
...
public class MathParser {
	...
	public MathParser(String expression) {
		this.expression = expression.replaceAll("\\) *\\(", "\\)\\*\\(");// ajoute un * entre deux parenthèses qui se suivent sans
																			// opérateur entre elles. Les espaces entre ) et ( sont ignorés
	}
	...
	private Term nextTerm(List<Term> terms) {
		...
		if (nextOperatorIndex == -1) {
			...
			return numberString.isEmpty() ? null : parseNumber(numberString, false);
		}
	 ...
	 *
	 * @return la position de l'un des caractères donnés, ou <code>-</code> si aucun de ces caractères n'existe entre la position
	 *         <code>pos</code> et la fin de l'expression à évaluer.
	 */
